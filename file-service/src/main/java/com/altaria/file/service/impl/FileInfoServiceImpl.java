...
public class FileInfoServiceImpl implements FileInfoService {
    ...
    public Result removeFile(List<Long> ids, Long uid) {
        ...
        for (String md5 : md5UrlMap.keySet()){
            if (md5Count.get(md5) == md5Total.get(md5)){
        ...
        minioService.deleteFile(delUrls);
    ...
    public Result upload(Long uid, Long fid, Long pid, MultipartFile file, String md5, Integer index, Integer total) {
        ...
        if (index == 0) {
            ...
            if (fileByMd5s != null && !fileByMd5s.isEmpty()) {
                ...
                Space space = spaceService.getUsedSpace(uid);
        ...
        // 0 ~ n - 1分片处理
        Space usedSpace = spaceService.getUsedSpace(uid);
                        ...
                        try {
                            ...
                            try {
                                if (type.compareTo(FileType.VIDEO.getType()) == 0){ // 转码视频
                                    ...
                                    // 更新数据库
                                    int i = fileInfoMapper.updateURLAndCoverByMd5(minioId + suffix, minioId + "_.jpg", md5);
                                    if (i <= 0){
                                    ...
                                    minioService.upLoadFile(minioId + suffix, new FileInputStream(data), contentType);
                                    // 更新数据库
                                    int i = fileInfoMapper.updateURLAndCoverByMd5(minioId + suffix, null, md5);
                                    if (i <= 0){
                                        log.error("数据库更新失败, 文件id: {},用户id: {}", dbId, uid);
                                    }
                                }
    ...
    private void saveFile(FileInfo saveFile) throws Exception {
        ...
        try {
            ...
            if (insert > 0) {
                ...
                spaceService.updateSpace(saveFile.getUid(), saveFile.getSize());
                return;
            }
